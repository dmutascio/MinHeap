...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void moveItem(int currLoc, int newLoc) {
		Decreaser<T> temp;
		ticker.tick();
		...
		array[newLoc]=array[currLoc];
		ticker.tick();
		array[newLoc].loc=newLoc;
		ticker.tick();
		
		array[currLoc]=temp;
		ticker.tick();
		array[currLoc].loc=currLoc;
		ticker.tick();
	}
	...
	void decrease(int loc) {
		...
		if(array[loc].getValue().compareTo(array[parent].getValue())<0) {
			ticker.tick();
			moveItem(loc, parent);
			ticker.tick();
			decrease(parent);
	...
	private void heapify(int where) {
		...
		if (LKid>size){
			...
			ticker.tick();
			if ((array[parent].getValue().compareTo(array[LKid].getValue()) > 0)) {
				moveItem(LKid, parent);
				ticker.tick();
				heapify(LKid);
				ticker.tick();
			}
		}else{
			if ((array[parent].getValue().compareTo(array[RKid].getValue()) < 0) &&
				(array[parent].getValue().compareTo(array[LKid].getValue()) < 0)) {
					return;
				}
			
			if ((array[parent].getValue().compareTo(array[LKid].getValue()) > 0) && 
				((array[parent].getValue().compareTo(array[RKid].getValue()) > 0))) {
				
				
				if ((array[LKid].getValue().compareTo(array[RKid].getValue()) < 0) && 
					(array[RKid]!=null)) {
					moveItem(LKid, parent);
					ticker.tick();
					heapify(LKid);
